...
def process_image(train_noisy):
    ...
    for h in range(3, train_noisy.size(2) - 3):
        for w in range(3, train_noisy.size(3) - 3):
            ...
            # 计算子图像的标准差并添加到列表中
            std = torch.std(sub_images.view(batch_size, -1), dim=1).view(-1, 1, 1)
...
def train(opt,model,device,train_dataloader,val_dataloader):
    ...
    for epoch in (range(1,epoch_num+1)):
        ...
        for index, train_data in enumerate(tqdm.tqdm(train_dataloader)):
            
            
            noise_data,clear_data = train_data
            std = process_image(noise_data)
            
            std = std.float()
            std = std.to(device)
            
            input_noisy = noise_data        # torch.Tensor
            input_clear = clear_data
            ...
            loss = loss_main(input_noisy, input_noisy_pred, clean, clean1, clean2, clean3, noise_b, noise_b1, noise_b2, noise_b3, noise_w, noise_w1, noise_w2,std,gamma)
